...
children.push(new Paragraph({
  ...
  children: [new TextRun({ text: "코더 코딩북 (Coding Book)", font: { name: FONT_KR }, size: 28, bold: true })]
}));
children.push(new Paragraph({
  alignment: AlignmentType.CENTER,
...
children.push(para("본 라벨링의 목적은 AI 모델 학습에 사용할 고품질 정답 데이터(Gold Standard)를 구축하는 것입니다. 코더의 판단이 곧 AI 모델의 학습 기준이 되므로, 일관성과 정확성이 가장 중요합니다."));
...
children.push(para("파일럿 분석 결과, 코더가 '사업을 수행하는 주체(제공자)'의 관점에서 SDG를 선택해 SDG10(불평등)을 과다 선택하는 구조적 혼동 패턴이 재현되었다. 1차 SDG 선택 직전 반드시 아래 3단계를 순서대로 확인한다."));
...
children.push(bullet("다른 코더와 라벨링 결과를 사전에 공유하거나 상의하지 마세요. (IAA 오염)"));
...
children.push(bullet("파일럿 완료 후 전체 코더가 참여하는 캘리브레이션 세션 1회 진행 (약 1시간)"));
...
children.push(para("모든 코더의 독립 라벨링이 완료된 후, 담당 연구자가 Fleiss' Kappa를 산출합니다. 전체 평균 및 SDG별 Per-class Kappa를 모두 산출합니다. v1.5부터는 본 라벨링 전에 v2 파일럿 모드(3인 동일 50건, blind) 재라벨링으로 κ를 선행 검증합니다."));
{
  const rows = [];
  rows.push(new TableRow({ children: [
    headerCell("구분", 3500),
...
children.push(bullet("코더 간 1차 SDG가 다른 케이스를 추출합니다."));
children.push(bullet("각 코더가 판단 근거 메모를 제출합니다."));
children.push(bullet("전체 코더가 모여 토론 후 다수결(2/3 이상 동의) 또는 합의로 최종 라벨을 확정합니다."));
...
children.push(bullet("② IAA 산출 시 HOLD 케이스는 제외하고 계산합니다. HOLD 비율은 코더별로 별도 집계하여 기록합니다."));
children.push(bullet("③ HOLD 해소 절차: 전체 코더 토론 → 2/3 이상 동의 시 확정 → 합의 불가 시 외부 전문가 중재. 해소 결과는 Gold Standard 데이터셋에 포함됩니다."));
